...
NICKNAME	=	caugusta
DATA_DIR	=	~/data
MANDATORY	=	nginx wordpress mariadb
BONUS		=
...
all: add_dns_to_host
	...
	@docker-compose -f  srcs/docker-compose.yml up --build -d ${MANDATORY}    # запускаем в фоне

bonus: all
	@docker-compose -f  srcs/docker-compose.yml up --build -d ${BONUS}    # запускаем в фоне
...
clean: down
	@echo "Очистка конфигурации ${NAME}..."
# сносит все (неиспользуемые контейнеры, образы, кэш), кроме волиумов
